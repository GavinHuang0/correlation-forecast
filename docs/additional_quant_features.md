...
\qquad
\mathtt{lagged\_realized\_volatility}_{a,t}
=
\sigma^{RV}_{a,t-1}.
```
...
```math
\mathtt{vix\_lag1}_t=VIX_{t-1},
\qquad
\mathtt{vix\_change\_lag1}_t=VIX_{t-1}-VIX_{t-2}.
```
...
```math
\mathtt{treasury}_{m,\mathrm{lag2},t}=Y_{m,t-2},
\qquad
